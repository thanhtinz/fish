...
| 8 | Resource and text extraction | **Built** | `graph`, with stable node ids and shape-based classification, over `resource`: properties, Android `strings.xml`, Apple `.strings`, gettext `.po` and JSON, each written back in place so comments and formatting survive, plus INI with section-qualified keys, Unreal's binary `.locres`, and Ren'Py's generated translation files - where the game's own script is deliberately read-only, because rewriting it is not how Ren'Py is localized. |
