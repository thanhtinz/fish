...
the file and a line number does not.

## What is named but not read

Listed rather than passed over, each with what it is: `classes.dex`, `resources.arsc`, Unreal's
`.locres`, Unity asset bundles, Godot `.pck`, Ren'Py `.rpa`. A translator who cannot see that a
game keeps half its dialogue in one of those will conclude the game is half translated when it is
not.

The DEX string pool and Unreal's `.locres` are both documented formats and both readable in
principle. They are not read yet, and the list says so rather than staying quiet.
